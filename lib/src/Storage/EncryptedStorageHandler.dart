...
class EncryptedStorageHandler implements IStorageHandler {
  ...
  Future<void> _initializeCache() async {
    try {
      ...
      Metriqus.verboseLog('🔧 [STORAGE] Cache initialization completed');
      Metriqus.verboseLog(
        '🔧 [STORAGE] Cached keys: ${_existsCache.keys.toList()}',
      );
      Metriqus.verboseLog(
        '🔧 [STORAGE] Attribution key in cache: ${_existsCache.containsKey("metriqus_last_send_attribution_date")}',
      );
    } catch (e) {
  ...
  Future<void> _ensureUserIdExists() async {
    try {
      ...
      if (!_cache.containsKey(userIdKey) || _cache[userIdKey]!.isEmpty) {
        ...
        Metriqus.verboseLog(
          '🔧 [STORAGE] User ID will be generated by UniqueUserIdentifier class',
        );
      } else {
        Metriqus.verboseLog(
          '🔧 [STORAGE] Existing user ID found: ${_cache[userIdKey]}',
        );
      }
    } catch (e) {
      Metriqus.errorLog('🔧 [STORAGE] Error ensuring user ID: $e');
    }
  }
        ...
        if ((saveKey == "metriqus_current_events" ||
          ...
          if (decryptedData.startsWith('[]') && decryptedData.length > 2) {
            ...
            try {
              ...
              Metriqus.verboseLog(
                '🔧 [STORAGE] Fixed malformed JSON file for $saveKey',
              );
            } catch (e) {
              Metriqus.errorLog(
                '🔧 [STORAGE] Error fixing malformed JSON file: $e',
              );
            }
          } else {
  ...
  void saveFile(String saveKey, String saveData) {
    try {
      ...
      if (saveKey == "metriqus_current_events" && saveData != "[]") {
        Metriqus.verboseLog(
          '🔧 [STORAGE] Saving current events: ${saveData.length > 100 ? saveData.substring(0, 100) + "..." : saveData}',
        );
      }
    } catch (e) {
  ...
  String readFile(String saveKey) {
    ...
    if (_cache.containsKey(saveKey)) {
      ...
      if (saveKey == "metriqus_current_events") {
        Metriqus.verboseLog(
          '🔧 [STORAGE] Reading current events from cache: ${value.length > 100 ? value.substring(0, 100) + "..." : value}',
        );
      }
      return value;
    }
    ...
    if (!_cacheInitialized) {
      ...
      Metriqus.verboseLog(
        '🔧 [STORAGE] Cache not initialized for key: $saveKey, returning empty',
      );
      return "";
    }
    ...
    Metriqus.verboseLog(
      '🔧 [STORAGE] Key not found in cache: $saveKey, returning empty',
    );
    return "";
  }
  ...
  bool checkKeyExist(String saveKey) {
    ...
    if (_existsCache.containsKey(saveKey)) {
      ...
      Metriqus.verboseLog(
        '🔧 [STORAGE] Found key in cache: $saveKey = $exists',
      );
      return exists;
    }
    ...
    if (!_cacheInitialized) {
      _preloadKey(saveKey); // Trigger async load
      Metriqus.verboseLog(
        '🔧 [STORAGE] Cache not initialized for key existence check: $saveKey, returning false',
      );
      return false;
    }
    ...
    Metriqus.verboseLog(
      '🔧 [STORAGE] Key existence not found in cache: $saveKey, returning false',
    );
    return false;
  }
